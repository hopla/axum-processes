...
void *thread(void *vargp)
{
  ...
  while (!ExitApplication)
  {
    ...
    else
    {
      ...
      {
        db_read_src_config(ObjectNr, ObjectNr);
      }
      if (DestinationConfigurationChanged)
      {
        db_read_dest_config(ObjectNr, ObjectNr);
      }
      if (ModuleConfigurationChanged)
      {
        db_read_module_config(ObjectNr, ObjectNr);
      }
      if (BussConfigurationChanged)
      {
        db_read_buss_config(ObjectNr, ObjectNr);
      }
      if (MonitorBussConfigurationChanged)
      {
        db_read_monitor_buss_config(ObjectNr, ObjectNr);
      }
      if (ExternSourceConfigurationChanged)
      {
        db_read_extern_src_config(ObjectNr, ObjectNr);
      }
