      ...
      if (InterfaceUsedForTransmit != NULL)
      {
        if (InterfaceUsedForTransmit->TransmitCallback != NULL)
        {
          ...
          if (!FilterActive)
          {
            ...
            case CAN:
            {
              TRACE_PACKETS(printf("[CAN] SendMambaMessage(0x%08lX, 0x%08lX, 0x%04X, %d)\n", ToAddress, FromAddress, MessageType, DataLength);)
            }
            break;
            case UART:
            {
              ...
              TRACE_PACKETS(printf("[UART] SendMambaMessage(0x%08lX, 0x%08lX, 0x%04X, %d)\n", ToAddress, FromAddress, MessageType, DataLength);)
            }
            break;
            case UART_CAN:
            {
              ...
              TRACE_PACKETS(printf("[UART_CAN] SendMambaMessage(0x%08lX, 0x%08lX, 0x%04X, %d)\n", ToAddress, FromAddress, MessageType, DataLength);)
            }
            break;
            case ETHERNET:
            {
              ...
              TRACE_PACKETS(printf("[ETHERNET] SendMambaMessage(0x%08lX, 0x%08lX, 0x%04X, %d)\n", ToAddress, FromAddress, MessageType, DataLength);)
            }
            break;
            case FIREWIRE:
            {
              TRACE_PACKETS(printf("[FIREWIRE] SendMambaMessage(0x%08lX, 0x%08lX, 0x%04X, %d)\n", ToAddress, FromAddress, MessageType, DataLength);)
            }
            break;
            case TCPIP:
            {
              ...
              TRACE_PACKETS(printf("[TCPIP] SendMambaMessage(0x%08lX, 0x%08lX, 0x%04X, %d)\n", ToAddress, FromAddress, MessageType, DataLength);)
            }
            break;
            default:
            {
              printf("MambaMessage could not be send, no interface found for 0x%08lX!\n", ToAddress);
            }
            break;
            }
          }
...
void STDCALL DecodeRawMambaNetMessage(unsigned char *Buffer, unsigned char BufferLength, int FromInterfaceIndex, unsigned char FromHardwareAddress[16])
{
  ...
  for (unsigned int cntAddress=0; cntAddress<AddressTableCount; cntAddress++)
  {
...
void ProcessMambaNetMessage(unsigned long int ToAddress, unsigned long int FromAddress, unsigned char Ack, unsigned long int MessageID, unsigned int MessageType, unsigned char *Data, unsigned char DataLength, int FromInterfaceIndex, unsigned char FromHardwareAddress[16])
{
  ...
  if (FromAddress != DefaultObjects->MambaNetAddress)
  {
    ...
    {  //Gateway function
      for (unsigned int cntInterface=0; cntInterface<InterfaceCount; cntInterface++)
      {
        if (((int)cntInterface) != FromInterfaceIndex)
        {
    ...
    if ((ToAddress == 0x10000000) || (ToAddress == DefaultObjects->MambaNetAddress))
    {
      ...
      case CAN:
      {
        TRACE_PACKETS(printf("[CAN] ProcessMambaNetMessage(0x%08lX, 0x%08lX, 0x%06lX, 0x%04X, %d)\n", ToAddress, FromAddress, MessageID, MessageType, DataLength);)
      }
      break;
      case UART:
      {
        TRACE_PACKETS(printf("[UART] ProcessMambaNetMessage(0x%08lX, 0x%08lX, 0x%06lX, 0x%04X, %d)\n", ToAddress, FromAddress, MessageID, MessageType, DataLength);)
      }
      break;
      case UART_CAN:
      {
        TRACE_PACKETS(printf("[UART_CAN] ProcessMambaNetMessage(0x%08lX, 0x%08lX, 0x%06lX, 0x%04X, %d)\n", ToAddress, FromAddress, MessageID, MessageType, DataLength);)
      }
      break;
      case ETHERNET:
      {
        TRACE_PACKETS(printf("[ETHERNET] ProcessMambaNetMessage(0x%08lX, 0x%08lX, 0x%06lX, 0x%04X, %d)\n", ToAddress, FromAddress, MessageID, MessageType, DataLength);)
      }
      break;
      case FIREWIRE:
      {
        TRACE_PACKETS(printf("[FIREWIRE] ProcessMambaNetMessage(0x%08lX, 0x%08lX, 0x%06lX, 0x%04X, %d)\n", ToAddress, FromAddress, MessageID, MessageType, DataLength);)
      }
      break;
      case TCPIP:
      {
        TRACE_PACKETS(printf("[TCPIP] ProcessMambaNetMessage(0x%08lX, 0x%08lX, 0x%06lX, 0x%04X, %d)\n", ToAddress, FromAddress, MessageID, MessageType, DataLength);)
      }
      break;
      default:
      {
        TRACE_PACKETS(printf("[---] ProcessMambaNetMessage(0x%08lX, 0x%08lX, 0x%06lX, 0x%04X, %d)\n", ToAddress, FromAddress, MessageID, MessageType, DataLength);)
      }
      break;
      }
      ...
      { //Address Reservation Information
        ...
        { //Reservation info.
          ...
          if ((!Found) && (FreeSlot != -1))
          {
            ...
            if (FreeSlot>((int)(AddressTableCount-1)))
            {
              AddressTableCount = FreeSlot+1;
            }
            ...
            if (FromHardwareAddress != NULL)
            {
              ...
              case UART:
              {
                TRACE_ADDRESSTABLE(printf("[UART] Added@%ld ManID:0x%04X, ProdID:0x%04X, UniqID:0x%04X, MambaNetAddr:0x%08lX HwAddr: COM%d\n", FreeSlot, ReceivedManufacturerID, ReceivedProductID, ReceivedUniqueIDPerProduct, ReceivedMambaNetAddress, FromHardwareAddress[0]);)
              }
              break;
              case UART_CAN:
              {
                TRACE_ADDRESSTABLE(printf("[UART_CAN] Added@%ld ManID:0x%04X, ProdID:0x%04X, UniqID:0x%04X, MambaNetAddr:0x%08lX HwAddr: COM%d\n", FreeSlot, ReceivedManufacturerID, ReceivedProductID, ReceivedUniqueIDPerProduct, ReceivedMambaNetAddress, FromHardwareAddress[0]);)
              }
              break;
              case ETHERNET:
              {
                TRACE_ADDRESSTABLE(printf("[ETHERNET] Added@%ld ManID:0x%04X, ProdID:0x%04X, UniqID:0x%04X, MambaNetAddr:0x%08lX HwAddr:%02X:%02X:%02X:%02X:%02X:%02X\n", FreeSlot, ReceivedManufacturerID, ReceivedProductID, ReceivedUniqueIDPerProduct, ReceivedMambaNetAddress, FromHardwareAddress[0], FromHardwareAddress[1], FromHardwareAddress[2], FromHardwareAddress[3], FromHardwareAddress[4], FromHardwareAddress[5]);)
              }
              break;
              case FIREWIRE:
              {
            ...
            else
            {
              ...
              case UART:
              {
                TRACE_ADDRESSTABLE(printf("[UART] Added@%ld ManID:0x%04X, ProdID:0x%04X, UniqID:0x%04X, MambaNetAddr:0x%08lX HwAddr: NULL\n", FreeSlot, ReceivedManufacturerID, ReceivedProductID, ReceivedUniqueIDPerProduct, ReceivedMambaNetAddress);)
              }
              break;
              case UART_CAN:
              {
                TRACE_ADDRESSTABLE(printf("[UART_CAN] Added@%ld ManID:0x%04X, ProdID:0x%04X, UniqID:0x%04X, MambaNetAddr:0x%08lX HwAddr: NULL\n", FreeSlot, ReceivedManufacturerID, ReceivedProductID, ReceivedUniqueIDPerProduct, ReceivedMambaNetAddress);)
              }
              break;
              case ETHERNET:
              {
                TRACE_ADDRESSTABLE(printf("[ETHERNET] Added@%ld ManID:0x%04X, ProdID:0x%04X, UniqID:0x%04X, MambaNetAddr:0x%08lX HwAddr: NULL\n", FreeSlot, ReceivedManufacturerID, ReceivedProductID, ReceivedUniqueIDPerProduct, ReceivedMambaNetAddress);)
              }
              break;
              case FIREWIRE:
              {
              ...
              case TCPIP:
              {
                TRACE_ADDRESSTABLE(printf("[TCPIP] Added@%ld ManID:0x%04X, ProdID:0x%04X, UniqID:0x%04X, MambaNetAddr:0x%08lX HwAddr: NULL\n", FreeSlot, ReceivedManufacturerID, ReceivedProductID, ReceivedUniqueIDPerProduct, ReceivedMambaNetAddress);)
              }
              break;
              case NO_INTERFACE:
              {
                TRACE_ADDRESSTABLE(printf("[NO_INTERFACE] Added@%ld ManID:0x%04X, ProdID:0x%04X, UniqID:0x%04X, MambaNetAddr:0x%08lX HwAddr: NULL\n", FreeSlot, ReceivedManufacturerID, ReceivedProductID, ReceivedUniqueIDPerProduct, ReceivedMambaNetAddress);)
              }
              break;
              }
            }
        ...
        {  //respone
              ...
              (DefaultObjects->UniqueIDPerProduct == ReceivedUniqueIDPerProduct))
          {
            ...
            if (DefaultObjects->MambaNetAddress == 0x00000000)
            {
              DefaultObjects->Services &= 0x7F; //address not valid
            }
            else
            {
              if (ReceivedNodeServices&0x80)
              ...
              {
                DefaultObjects->Services &= 0x7F; //address not validated
              }
            }
      ...
      {  //Object message
        ...
        case  MAMBANET_OBJECT_ACTION_GET_INFORMATION:
        {
          ...
          {  //Only for the non-standard objects
            ...
            SendMambaNetMessage(FromAddress, DefaultObjects->MambaNetAddress, Ack, MessageID, 1, TransmitBuffer, cntSize+5);
            MambaNetMessageProcessed = 1;
          }
          ...
          else
          {
            ...
            SendMambaNetMessage(FromAddress, DefaultObjects->MambaNetAddress, Ack, MessageID, 1, TransmitBuffer, 20);
            MambaNetMessageProcessed = 1;
          }
        }
        break;
        ...
        case MAMBANET_OBJECT_ACTION_GET_ENGINE_ADDRESS:
        {
          ...
          {  //Only for the non-standard objects
            ...
            SendMambaNetMessage(FromAddress, DefaultObjects->MambaNetAddress, Ack, MessageID, 1, TransmitBuffer, 9);
            MambaNetMessageProcessed = 1;
          }
          else if ((ObjectNr>=(1024+NumberOfCustomObjects)) && (ObjectNr<(1024+DefaultObjects->NumberOfObjects)))
        ...
        case  MAMBANET_OBJECT_ACTION_GET_OBJECT_FREQUENCY:
        {
          ...
          {  //Only for the non-standard objects
            if (CustomObjectInformation[ObjectNr].Services != 0x00)
            {
              ...
              SendMambaNetMessage(FromAddress, DefaultObjects->MambaNetAddress, Ack, MessageID, 1, TransmitBuffer, 6);
              MambaNetMessageProcessed = 1;
            }
          }
          else if ((ObjectNr>=(1024+NumberOfCustomObjects)) && (ObjectNr<(1024+DefaultObjects->NumberOfObjects)))
        ...
        case  MAMBANET_OBJECT_ACTION_GET_SENSOR_DATA:
        {
          ...
          {  //Only for the standard obects.
            ...
            {  //Description
              ...
              SendMambaNetMessage(FromAddress, DefaultObjects->MambaNetAddress, Ack, MessageID, 1, TransmitBuffer, 69);
            }
            break;
            ...
            {  //ManufacturerID
              ...
              SendMambaNetMessage(FromAddress, DefaultObjects->MambaNetAddress, Ack, MessageID, 1, TransmitBuffer, 7);
            }
            break;
            case 3:
            {  //ProductID
            ...
            { //Hardware parent
              ...
              SendMambaNetMessage(FromAddress, DefaultObjects->MambaNetAddress, Ack, MessageID, 1, TransmitBuffer, 11);
            }
            break;
            case 16:
            { //Service requested
        ...
        case  MAMBANET_OBJECT_ACTION_GET_ACTUATOR_DATA:
        {
          ...
          {  //Only for the standard obects.
            ...
            {  //Name
              ...
              SendMambaNetMessage(FromAddress, DefaultObjects->MambaNetAddress, Ack, MessageID, 1, TransmitBuffer, 37);
            }
            break;
            case 12:
        ...
        case  MAMBANET_OBJECT_ACTION_SET_ACTUATOR_DATA:
        {
          if (ObjectNr<1024)
          {
            ...
            else if (MessageID)
            {
              ...
              SendMambaNetMessage(FromAddress, DefaultObjects->MambaNetAddress, Ack, MessageID, 1, TransmitBuffer, 4);
            }
          }
          else
          {
...
unsigned char STDCALL Data2ASCIIString(char ASCIIString[128], unsigned char DataType, unsigned char DataSize, unsigned char *PtrData)
{
  ...
  case UNSIGNED_INTEGER_DATATYPE:
  {
    ...
    sprintf(ASCIIString, "%ld", TempData);
    ReturnValue = 0;
  }
  break;
  ...
  case OCTET_STRING_DATATYPE:
  {
    ...
    sprintf(ASCIIString, "%s", TempString);
    ReturnValue = 0;
  }
  break;
  case FLOAT_DATATYPE:
...
void STDCALL UnregisterMambaNetInterface(int InterfaceIndex)
{
  ...
  {
    AddressTableCount = 0;
    for (unsigned long cntAddress=0; cntAddress<AddressTableCount; cntAddress++)
    {
  ...
  if (InterfaceCount == ((unsigned int)InterfaceIndex+1))
  {
    InterfaceCount--;
  }
}
...
void STDCALL MambaNetReservationTimerTick()
{
  ...
  for (unsigned long cntAddressEntry=0; cntAddressEntry<AddressTableCount; cntAddressEntry++)
  {
